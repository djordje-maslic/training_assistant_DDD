  ...
  Widget build(BuildContext context) {
    return BlocConsumer<ExerciseFormBloc, ExerciseFormState>(
      ...
      listener: (context, state) {
        context.formSets = state.exercise.setsList.value.fold(
            (f) => listOf<SetItemPrimitive>(),
            (setItemList) =>
                setItemList.map((_) => SetItemPrimitive.fromDomain(_)));
      },
      buildWhen: (p, c) =>
        ...
        return ListTile(
          ...
          onTap: () {
            context.formSets.isEmpty()
                ? context.formSets =
                    context.formSets.plusElement(SetItemPrimitive.empty())
                : context.formSets = context.formSets.plusElement(context
                    .formSets[context.formSets.lastIndex]
